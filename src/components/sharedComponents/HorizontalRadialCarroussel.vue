<template>
  <div ref="carrousselContainer" class="horizontal-carroussel">
    ...
    <ProductCard
      ...
      :style="{
        transform:
          index === currentIndex
            ? ''
            : `translate(${getItemXTranslation(index)}%, -50%) scale(${getItemScale(index)})`,
    ...
    <button
      ...
      @:click="rotateCarrousselTo('right')"
    >
      <SvgIcon
        :name="'Arrow'"
        :color="'white'"
...
function isItemVisible(index: number): boolean {
  ...
  return (
    relativeIndex >= currentIndex.value - sideVisibleItems &&
    relativeIndex <= currentIndex.value + sideVisibleItems
  );
}
...
function getEquivalentIndex(index: number): number {
  if (
    currentIndex.value <= 0 + sideVisibleItems &&
    index >= numberOfItems - 1 - (sideVisibleItems - currentIndex.value)
  ) {
    return index - numberOfItems;
  }
  if (
    currentIndex.value >= numberOfItems - 1 - sideVisibleItems &&
    index <= sideVisibleItems
  ) {
    return index + numberOfItems;
  }
  return index;
